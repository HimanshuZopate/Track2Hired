...
function AnimatedButton({ children, loading, disabled, type = 'button', onClick, className = '' }) {
  return (
    <Motion.button
      type={type}
      onClick={onClick}
      ...
      className={`group relative flex h-12 w-full items-center justify-center overflow-hidden rounded-xl bg-gradient-to-r from-blue-500 to-purple-500 px-4 text-sm font-semibold text-white transition-all duration-300 hover:shadow-[0_0_25px_rgba(59,130,246,0.5)] disabled:cursor-not-allowed disabled:opacity-70 ${className}`}
    >
